...
\begin{center}
    University of Connecticut \\ Department of Mathematics \\ Supervisor: Dr. Jeremy Teitelbaum \\ Advisor: Professor Iddo Ben-Ari
\end{center}
...
\begin{figure}[ht]
    ...
    \begin{subfigure}[]{0.5\textwidth}
         \centering
         \caption{Source: Data Driven Investor \cite{clusteringvassociation}}
     \end{subfigure}
    \label{fig1}
\end{figure}
...
\begin{table}[ht]
    ...
    \begin{tabular}{c c c c c}
         ...
         (mm) & (mm) & (mm) & (g) \\
         \hline
         50.2 & 18.7 & 198 & 3775 & Gentoo \\
         39.5 & 17.4 & 186 & 3800  & Adelie \\
         44.9 & 13.8 & 212 & 4750 & Chinstrap \\
         52.2 & 17.1 & 228 & 5400 & Chinstrap \\
         50.8 & 19 & 210 & 4100 & Gentoo \\
         42.5 & 20.7 & 197 & 4500 & Adelie
    \end{tabular}
    \caption{Penguin Data}
    \label{table_penguin_data}
\end{table}

In this example, the culmen length, culmen depth, flipper length, and body mass are the predictors. Based on these variables, we want the model to predict which species the penguin is. So, each body measurement is a predictor and the species is the response variable. In supervised learning, we often have training and testing groups for our data. Training and test groups are a common practice in machine learning to both classify existing data points and assess the underlying accuracy of the model on known data. Training data is data used to teach our model how to estimate the response variable. Once the model is trained, we can apply the algorithm to the testing data which the model has not previously seen. Since we already have the response variable for this set in the original data, we can compare the model's predicted $y$-values, denoted $\hat y$, with the actual $y$-values. In machine learning, it is common to see the data split into about 80\% training and 20\% testing data, but these values can be manually specified in the code \cite{trainingvtest}. Suppose we collect data for 100 students at the end of the school year. We take 80 students and train the model based on these observations. Then, we use the remaining 20 students as the test set to see how well our model performs. Splitting the data into training and test sets is an important tool in model validation.

\subsubsection{REGRESSION VS. CLASSIFICATION}

Supervised learning can be categorized into either regression or classification problems. Generally, this has to do with the fact that data is either qualitative (categorical) or quantitative (numerical). Regression typically uses a quantitative response variable. In regression, we aim to create a model that uses features to accurately predict the response variable. We may want to see how a population's access to clean drinking water impacts their average life expectancy. Linear regression can be used in this case to see whether a correlation between the two variables exist and how strong that correlation is. We can look at certain statistics that assess how accurate the model is at predicting the response variable as well as investigate whether the relationship is linear or non-linear. On the other hand, classification is common with a quantitative response variable. In these types of problems, the data is grouped into specific categories based on the given features. The model is then able to predict the category of an unknown data point. Similar to regression, we can use various tools to validate the model and adjust parameters to increase accuracy.

We have now discussed a short history of machine learning, the difference between supervised and unsupervised learning, and the two approaches to supervised learning, namely regression and classification. Under the umbrella of classification problems in supervised learning fall support vector machines (SVMs). Developed in the 1990s, support vector machines are an approach to classification problems now widely used among data scientists. In the following paper we will cover SVMs, including two classification methods that are the basis of support vector machines, how one can be implemented among a data set, practical applications of SVMs, and the surrounding documentation.
...
Suppose we look at a few points with a separating hyperplane of $0=f(x_1,x_2)=\beta_0+\beta_1x_1+\beta_2x_2$, pictured in Figure ~\ref{supportvectors}. We assume the point $(a_1,a_2)$ in the set of the positive class labels is the closest point to the set of points with a negative class label. To compute the perpendicular distance $d$ from the point $(a_1,a_2)$ to $f(x_1,x_2)=0$, we first find a normal vector to $f$. Through concepts in calculus, we find the gradient of the hyperplane calculated by $(dx_1,dx_2)$ gives the normal vector $\vec{n}=\expval{\beta_1, \beta_2}$ to $f$. Let $(b_1,b_2)$ be any point on the hyperplane. Then $\vec{v}=(a_1-b_1,a_2-b_2)$. Through trigonometry, the distance $d$ is equivalent to $||\vec{v}|| \cos{\theta}$. Then we have
\begin{align*}
...
Though the maximal margin classifier is not a support vector machine quite yet, we may use the SVM module in the "e1071" library in $R$ to determine the equation of the line \cite{e1071}.
...
To find the equation of the line, we use the hyperplane function as outlined in \cite{teitelbaum2021svm} and translated into $R$:
\begin{minted}[breaklines]{R}
...
We can add a line to visualize a possible hyperplane by running the following line of code after we have constructed the graph, shown in Figure ~\ref{fig_svc_randomly_generated_w_line}.
...
The difference is a reference to the "dual problem", as there are two approaches to solving this problem. One approach, which we have previously outlined, fixes the variable \(C\) and the sum of \(\beta_j^2\) in order to maximize the margin. We are left with solving \[
    y_i(\beta_0+\beta_1x_{i1}+...+\beta_px_{ip}) \ge M(1-\epsilon_i)
    \]
subject to the conditions on \(\beta\) and \(\epsilon\). We could also solve this problem by switching our conditions as outlined in \cite{esl2}. If we fix the margin to be 1, we aim to minimize \(||\beta||\). The new problem becomes 
...
The constant defined in Equation ~\ref{eq_svc_initial} is replaced with the cost parameter of the Lagrange function in Equation ~\ref{eq_svc_final}. This cost parameter is the same as the cost variable defined in both the $R$ and Python $\textbf{svm}$ modules.
...
We can again validate our model using the test set. The same code as covered in the maximal margin classifier is used, and we again see there are no misclassified points. Thus, we have produced the best support vector classifier for our data, illustrated in Figure ~\ref{fig_svc_final_model}.
...
We then want to create a \textbf{numpy} array of the desired variables which will work with the SVM module. We will be removing the species (class label), island, and sex predictors. This will be the dataset we use in the support vector machine and is located in Figure ~\ref{fig_penguins_numpy}.
...
We then fit our model to specified cost and gamma values. We use the training sets to create the model then use the model to predict the species based on the culmen depth and culmen length in the test set. Finally, we print the accuracy of the model when used on the test set.
...
This gives us an output of 95.5\(\%\). Could we achieve a higher accuracy? The \textbf{GridSearch} function gives us a method to calculate the accuracies of many different cost and gamma combinations, much like the tuning function in $R$ \cite{svm_python_tuning}. The code below sets a list of cost and gamma values that the function fits to the training data. We can see the best parameters using the \textbf{best estimator} variable of the grid function.
...
The output of the code is shown in Figure ~\ref{fig_fit_cl_cd}. Recall that Adelie penguins have a class label of 0, Chinstrap of 1, and Gentoo of 2. The underlying color background shows the decision boundary of the classifier. For example, points that lie within the blue area would be classified as Adelie penguins. From the plot, we can see several outliers - a white point in the blue area, blue and red points in the white area, and a white point in the red area. Though there are some misclassified points, our first support vector machine has the potential for accurately determining a penguin's species based on the culmen length and culmen depth predictors due to the high model accuracy.
...
The output of the code is shown in Figure ~\ref{fig_fit_cl_fl}. Adelie penguins have a class label of 0 with Chinstrap and Gentoo being 1 and 2, respectively. We see most points are classified correctly. Due to the high model accuracy, this seems like a good choice of predictors for a support vector machine that predicts a penguin's species.
...
While the classifier using the culmen depth and body mass improved greatly, the best tuned machine using culmen length and body mass has a lower accuracy than our attempts with culmen length and both culmen depth and flipper length. As such, we will probably not use this model going forward.
...
The SVM modules in both programming languages are similar. Python's \textit{scikitlearn} and $R$'s \textit{e1071} both use a one-vs-one approach in multi-class models. The mathematical formulation for the Python classifier can be accessed through \cite{scikit_svm}. On the other hand, the $R$ module was based on $C/C++$ code. Thus, the mathematical formulation is said to follow that of Chih-Chung Chang and Chih-Jen Lin in \href{https://www.csie.ntu.edu.tw/~cjlin/papers/libsvm.ps.gz}{LIBSVM: a library for Support Vector Machines}. This file proved cumbersome to open and is not as accessible as that of \textit{scikitlearn}.

When it comes to plotting, Python offers flexible and intuitive tools. While the code can get quite long, the plots are aesthetic and customizable due to the versatile nature of Python programmers. There are options for plotting in $R$ such as \textit{ggplot2} that go beyond the elementary base plotting code but was not covered in this paper.
